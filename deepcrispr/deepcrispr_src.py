...
def custom_loss(labels_true, logits_est):
     cross_entropy = tf.reduce_sum(tf.cast(tf.math.count_nonzero(1-labels_true),tf.float32)*labels_true * -tf.log(logits_est + 1e-16)) + tf.reduce_sum(tf.cast(tf.math.count_nonzero(labels_true),tf.float32)*(1-labels_true) * -tf.log((1-logits_est) + 1e-16))
...
class DCModel:
    ...
    def train(self,x,y,batch_size=500,epochs=50):
        self.inputs_sg = tf.placeholder(dtype=tf.float32, shape=[None, 1, 23, 8])
        self.pred_ontar = build_ontar_model(self.inputs_sg)
        labels_pl= tf.placeholder(dtype=tf.float32, shape=[None])
        loss = custom_loss(labels_pl,self.pred_ontar) #tf.losses.softmax_cross_entropy(labels_pl,self.pred_ontar)
        dataset = Dataset(x,y)
        optimizer = tf.train.AdamOptimizer(0.001)
        train_op = optimizer.minimize(loss)
        self.sess.run(create_init_op())
        init = tf.initialize_all_variables()
        self.sess.run(init)
        for i in range(epochs):
            input_feed, target_feed = dataset.next_batch(batch_size)
            feed_dict_supervised = {self.inputs_sg: input_feed, labels_pl: target_feed}
            #feed_dict_supervised = {self.inputs_sg: x, labels_pl: y} #input_feed, labels_pl: target_feed}
            _, loss_train = self.sess.run([train_op, loss],feed_dict=feed_dict_supervised)
            #loss_val = self.sess.run(loss,feed_dict=feed_dict_supervised)
            if i % 10 == 0:
                print("Epoch:",i+1)
                print('Loss:',loss_train)

        # Now predict on the entire dataset
        yp = self.sess.run(self.pred_ontar, feed_dict={self.inputs_sg: x})
        print("Results if you just guessed")
        print(pd.Series(y).value_counts(normalize=True))
        print("Our results")
        print(sum(np.round(yp)==y)/len(y))
